...
TEXT-BUCKET DATA PATH (documented per task):
  stage6/data/natural/eval/<family>.jsonl — one row per natural-pool example
  with fields: example_id, text, token_ids (gemma-2 ids, NO BOS — extraction
  prepended BOS at forward time, stage5/code/extract.py), n_tokens, and
...
Default layers per concept: probe-card chosen layer (stage6/artifacts/
...
EVAL_DIR = STAGE_DIR.parent / "stage6" / "data" / "natural" / "eval"
...
def summarize(fam, cls, res, args, dirs):
    ...
    for li, layer in enumerate(res["layers"]):
        ...
        cfg = {"factors": args.factors, "fit_range": list(FIT_RANGE),
               ...
               "ymax_pos": YMAX_POS,
               "bucket_source": "stage6/data/natural/eval/<family>.jsonl"}
        if layer == 25:
