...
Parts
  ...
  (c) Selectivity-restore: everywhere-ablate concept c (ridge arm), add back
      ...
      (3_validation/artifacts/probe_cards.json), α = {0.5, 1, 2} × s95(chosen
...
Data paths (verified on disk 2026-07-02)
  * Natural positives / neutrals: 3_validation/data/natural/eval/<family>.jsonl
    (built by 3_validation/code/build_natural_eval.py FROM
    1_dataset/data/<family>/judged/judged_nat.jsonl — same texts, judge spans
  ...
  * Ablation targets t: ridge → common.dose_calib (unit-w score units);
    ...
    — 3_validation/code/score_natural.py proj().
...
EVAL_DIR = CP_DIR / "3_validation" / "data" / "natural" / "eval"
NATSCORES_DIR = CP_DIR / "3_validation" / "data" / "natscores"
PROBES_DIR = CP_DIR / "2_probes" / "probes"
CARDS_PATH = CP_DIR / "3_validation" / "artifacts" / "probe_cards.json"
        ...
        if fam not in self._eval_rows:
            ...
            if not path.exists():
                raise FileNotFoundError(
                    f"{path} missing — run 3_validation/code/build_natural_eval.py")
...
def base_config(ctx, args):
    return {"blocks": list(ctx.blocks), "probed": list(ctx.probed),
            ...
            "data": "3_validation/data/natural/eval/<fam>.jsonl (from stage4 "
                    "judged_nat.jsonl)"}
