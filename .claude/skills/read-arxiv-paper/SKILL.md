...
Fetch the url to a local .tar.gz file. A good location is `~/.cache/oracle-encodings/knowledge/{arxiv_id}.tar.gz`.

(If the file already exists, there is no need to re-download it).

### Part 3: Unpack the file in that folder

Unpack the contents into `~/.cache/oracle-encodings/knowledge/{arxiv_id}` directory.
...
As for the summary itself, remember that you're processing this paper within the context of the oracle-encodings repository, so most often we will be interested in how to apply the paper and its lessons to the oracle-encodings project. Therefore, you should feel free to "remind yourself" of the related oracle-encodings code by reading the relevant parts, and then explicitly make the connection of how this paper might relate to oracle-encodings or what are things we might be inspired about or try.