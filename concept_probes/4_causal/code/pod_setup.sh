...
# (natstats26.npz + 3_validation/artifacts/probe_cards.json are git-tracked and
#  arrive with the clone.)
for FAM in $FAMILIES; do
  echo "== [$FAM] probes =="
  run hf download "$WREPO" --include "families/$FAM/*" \
    --local-dir "$STAGING" --quiet
  run mkdir -p "$CP/2_probes/probes/$FAM"
  if [ "$DRYRUN" = "1" ]; then
    echo "DRYRUN: cp -a $STAGING/families/$FAM/. $CP/2_probes/probes/$FAM/"
  else
    cp -a "$STAGING/families/$FAM/." "$CP/2_probes/probes/$FAM/"
  fi
done
...
run mkdir -p "$CP/3_validation/data/natscores" "$CP/3_validation/data/natural"
if [ "$DRYRUN" = "1" ]; then
  echo "DRYRUN: cp -a $STAGING/natscores/. $CP/3_validation/data/natscores/"
  echo "DRYRUN: cp stage6_1/inputs/random_pool.jsonl into 3_validation/data/natural/"
else
  [ -d "$STAGING/natscores" ] && cp -a "$STAGING/natscores/." "$CP/3_validation/data/natscores/"
  ...
  if [ -d "$STAGING/stage6_1/inputs/eval" ]; then
    mkdir -p "$CP/3_validation/data/natural/eval"
    cp -a "$STAGING/stage6_1/inputs/eval/." "$CP/3_validation/data/natural/eval/"
  else
...
# Dataset paths start with data/<fam>/..., so --local-dir at 1_dataset/ lands
# them at concept_probes/1_dataset/data/<fam>/{judged,final}/ — the exact
# relative paths the harness reads.
for FAM in $FAMILIES; do
  echo "== [$FAM] stage4 judged/final =="
  run hf download "$DREPO" --repo-type dataset \
    --include "data/$FAM/final/*" --local-dir "$CP/1_dataset" --quiet
  run hf download "$DREPO" --repo-type dataset \
    --include "data/$FAM/judged/*" --local-dir "$CP/1_dataset" --quiet
done
...
sys.path.insert(0, os.path.join(repo, "concept_probes/4_causal/code"))
import common
...
for f in fams:
    ...
    if not (common.CP_DIR / "1_dataset" / "data" / f / "judged" /
